 ...
 * /api/admin/getAll:
 *   get:
 *     description: Get all admins data | JWT Autharization token required in the request header
 *     tags:
 *       - Admin Data
 *     security:
 *       - bearerAuth: []
...
router.get("/getAll", authAdminPrivateRoute, async (req, res) => {
  try {
 ...
 * /api/admin/get/{id}:
 *   get:
 *     description: Get Admin Data by user ID | JWT Autharization token required in the request header
 *     tags:
 *       - Admin Data
 *     parameters:
 *       - in: path
 ...
 *         schema:
 *           type: string
 *         description: User ID to retrieve Admin Data
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       '200':
...
router.get("/get/:id", authAdminPrivateRoute, async (req, res) => {
  try {
 ...
 * /api/admin/edit/{id}:
 *   put:
 *     description: Update Admin Data by user ID | JWT Authorization token required in the request header
 *     tags:
 *       - Admin Data
 *     parameters:
 *       - in: path
 ...
 *         name: userData
 *         required: true
 *         description: Object containing admin data to be updated
 *         schema:
 *           type: object
 *           properties:
 *             name:
 ...
 *     responses:
 *       '200':
 *         description: Successfully updated admin data
 *       '401':
 *         description: Unauthorized - JWT token is missing or invalid
 *       '404':
 *         description: User not found
...
router.put("/edit/:id", authAdminPrivateRoute, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Admin Data has been updated", data: user });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.put("/update/password/:id", authAdminPrivateRoute, async (req, res) => {
  try {
 ...
 * /api/admin/delete/{id}:
 *   delete:
 *     description: Delete Admin Data by user ID | JWT Authorization token required in the request header
 *     tags:
 *       - Admin Data
 *     parameters:
 *       - in: path
 ...
 *     responses:
 *       '200':
 *         description: Successfully deleted Admin Data
 *       '401':
 *         description: Unauthorized - JWT token is missing or invalid
 *       '404':
 *         description: User not found
